...
editWords = [
    "RETARD", "kys", "CHINK", "chinese", "swat", "die", "nuke", "selfbot",
    "faggot", "yellow", "Doxx", "fag", "doxx", "chinks", "chink"
]
...
async def on_message(message):
    for word in editWords:
        if word in message.content.lower() and filterWords:
            time.sleep(0.5)
            try:
                return await message.edit(content=".", delete_after=0.5)
            except:
                pass
...
[ Made by: valhalla#0001 ]```"""
    ...
    if setting.lower() == "filter":
        if filterWords:
            filterWords = False
            await ctx.message.edit(content=validTXT.format("filter", "off"),
                                   delete_after=10)
        else:
            filterWords = True
            await ctx.message.edit(content=validTXT.format("filter", "on"),
                                   delete_after=1)
        ...
        await ctx.message.edit(content=invalidTXT, delete_after=10)
...
[ Made by: valhalla#0001 ]```""".format(
            snipe_message_author[ctx.channel.id], f"#{ctx.channel.name}",
...
async def stream(ctx, *, message):
    ...
    print("[#] [ACTION]: Set streaming status in channel: \"{}\".".format(
        ctx.channel.name))
    await ctx.message.edit(content="""```ini
[ Made By: valhalla#0001 ]
...
[ Made by: valhalla#0001 ]```""".format(message),
                           delete_after=5)
...
async def purge(ctx, amount: int):
        ...
        try:
            if message.author == client.user:
                ...
                if (random.randint(0, 16) == 4
                    ):  #To avoid ratelimit/term, sleep every so often.
                    time.sleep(7.5)
                    slept += 1
            else:
                pass
    ...
    print("[#] [ACTION]: Finished purge in channel: \"{}\".".format(
        ctx.channel.name))
    await ctx.send("""```ini
[ Made By: valhalla#0001 ]
...
[ Made by: valhalla#0001 ]```""".format(deleted, slept, ratelimited),
                   delete_after=1)
